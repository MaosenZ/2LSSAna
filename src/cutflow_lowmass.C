...
void cutflow_lowmass(TString masspoint, Int_t input_channel){
  //low mass
  ...
  //TFile *f_totalbkg=new TFile("/scratchfs/atlas/zhoums/2LSSAna_output/output/Systematics/nominal/miniNtuple_promptSS.root");
  ...
  for(Int_t index=0;index<4;index++){
      for(Long64_t entry=0; entry<signal->GetEntries();entry++) {
          ...
          if(index==0 &&signal_mindR_l1j>fixed_cuts_min[0]&&signal_mindR_l1j<fixed_cuts_max[0]){
             numOfsignal[index]++;
             numOfwgtsignal[index]+=signal_weight; Err2Ofwgtsignal[index]+=signal_weight*signal_weight;
          }
          if(index==1 && signal_mindR_l1j>fixed_cuts_min[0]&&signal_mindR_l1j<fixed_cuts_max[0] &&
                         signal_m_ll>fixed_cuts_min[1]&&signal_m_ll<fixed_cuts_max[1]){
             numOfsignal[index]++;
             numOfwgtsignal[index]+=signal_weight; Err2Ofwgtsignal[index]+=signal_weight*signal_weight;
          }
          if(index==2 && signal_mindR_l1j>fixed_cuts_min[0]&&signal_mindR_l1j<fixed_cuts_max[0] &&
                         signal_m_ll>fixed_cuts_min[1]&&signal_m_ll<fixed_cuts_max[1] &&
                         signal_m_l1jj>fixed_cuts_min[2]&&signal_m_l1jj<fixed_cuts_max[2]){
             numOfsignal[index]++;
             numOfwgtsignal[index]+=signal_weight; Err2Ofwgtsignal[index]+=signal_weight*signal_weight;
          }
          if(index==3&&signal_mindR_l1j>fixed_cuts_min[0]&&signal_mindR_l1j<fixed_cuts_max[0] &&
                         signal_m_ll>fixed_cuts_min[1]&&signal_m_ll<fixed_cuts_max[1] &&
                         signal_m_l1jj>fixed_cuts_min[2]&&signal_m_l1jj<fixed_cuts_max[2] &&
